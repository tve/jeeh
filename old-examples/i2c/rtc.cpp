// Read out the DS3231 RTC module (and optionally also set it).
...
int main () {
    initBoard();
    i2cBus.init(I2C_PINS, 100);
    i2c::Dev ds3231 {i2cBus, 0x68};

    if (0) { // set the date
        DateTime now; // the time this code was compiled, that is ...
        uint8_t buf [] = { bin2bcd(now.ss), bin2bcd(now.mm), bin2bcd(now.hh),
                           1, // day of week, not used
                           bin2bcd(now.dy), bin2bcd(now.mo), bin2bcd(now.yr) };
    ...
    DateTime dt { bcd2bin(buf[6]), bcd2bin(buf[5]), bcd2bin(buf[4]),
                  bcd2bin(buf[2]), bcd2bin(buf[1]), bcd2bin(buf[0]) };
