...
void rdMem (uint16_t addr, void* buf, uint16_t len) {
    ...
    sram.ioReq(IO_START|IO_WRITE, hdr, sizeof hdr);
    sram.ioReq(IO_READ|IO_STOP, (uint8_t*) buf, len);
}

void wrMem (uint16_t addr, void const* buf, uint16_t len) {
    uint8_t hdr [] = { 0x02, (uint8_t) (addr>>8), (uint8_t) addr };
    sram.ioReq(IO_START|IO_WRITE, hdr, sizeof hdr);
    sram.ioReq(IO_WRITE|IO_STOP, (uint8_t*) buf, len);
}
...
void testSram () {
    ...
    sram.ioReq(IO_START|IO_WRITE|IO_STOP, hdr, sizeof hdr);
