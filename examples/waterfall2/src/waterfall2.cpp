...
template <typename T>
void initRadio(T& rf, uint8_t config, uint32_t freq) {
    ...
    rf915.setFrequency(freq);
    printf("Radio rev: %02x\r\n", rf.readReg(0x42));
}

// dump all radio registers
template <typename T>
void dumpRadioRegs(T& rf) {
    printf("   ");
    for (int i = 0; i < 16; ++i)
        printf("%3x", i);
...
template <typename T>
void setFreq(T& rf, uint32_t freq) {
...
template <typename T>
void sweepRadio(T& rf, uint32_t first, uint32_t step, int count, uint8_t usDelay, uint8_t buf[]) {
    uint32_t freq = first;
...
void waterfall(T& rf, int which) {
    ...
    while (true) {
        ...
        for (int y = 16; y < lcd.height; ++y) {
            ...
            if (checkBtn(btn1)) { debounceBtn(btn1); return; }
        }
...
int main () {
    ...
    while (true) {
      switch (which) {
      case 0:
        waterfall(rf915, which);
        break;
      case 1:
        waterfall(rf433, which);
        break;
      }
      which = (which+1)%2;
    }
